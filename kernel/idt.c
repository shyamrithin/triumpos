/*
 * ============================================================
 * Triumph OS - IDT Implementation
 * File: kernel/idt.c
 * Description: Sets up the Interrupt Descriptor Table with
 *              256 entries covering CPU exceptions and hardware
 *              interrupts. Loads the IDT into the CPU using
...
/*
 * idt_set_gate()
 * Fills one IDT entry with the address of a handler function.
 */
...
/*
 * idt_init()
 * Registers all 48 interrupt handlers and loads the IDT.
 */
void idt_init(void) {
    idt_ptr.limit = (sizeof(idt_entry_t) * IDT_ENTRY_COUNT) - 1;
    idt_ptr.base  = (uint32_t)&idt;

    /* CPU Exceptions — interrupts 0 to 31 */
    ...
    /* Hardware IRQs — interrupts 32 to 47 */
